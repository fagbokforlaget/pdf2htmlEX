...
bool ThumbRenderer::render_page(PDFDoc * doc, int pageno)
{
    ...
    page_width = html_renderer->html_text_page.get_width() * param.actual_dpi / DEFAULT_DPI;
    page_height = html_renderer->html_text_page.get_height() * param.actual_dpi / DEFAULT_DPI;
    h_res = (html_renderer->text_zoom_factor() * param.actual_dpi * 400) / param.fit_width;
    v_res = (html_renderer->text_zoom_factor() * param.actual_dpi * 300) / param.fit_height;
...
void ThumbRenderer::setMimeData(GfxState *state, Stream *str, Object *ref,
